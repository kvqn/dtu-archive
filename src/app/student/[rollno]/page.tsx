  ...
  return (
    <div className="flex w-full flex-col items-center pb-20">
      <div className="flex w-[90%] flex-col items-center gap-4">
        <h2 className="font-geologica text-center text-3xl">{name}</h2>
        <div className="font-geist grid w-full grid-cols-2 justify-evenly">
        ...
        <div className="flex w-full flex-col gap-6">
          {Array.from(new Set(grades.map((grade) => grade.semester))).map(
            (semester) => (
              <div
                key={semester}
                className="divide-y-black w-fulldivide-y divide-y-2 divide-black overflow-hidden rounded-xl border-2 border-black bg-emerald-100 shadow-lg dark:bg-emerald-800"
              >
                <div className="font-geologica p-2 text-center text-xl">
                  Semester {semester}
                </div>
                  ...
                  {grades
                    ...
                    .map((grade) => (
                      <div
                        ...
                        className="font-geist flex w-[90%] divide-x-2 divide-black overflow-hidden rounded-md border-2 border-black text-sm"
                      >
                        <div className="w-[25%] bg-teal-200 py-1 text-center font-medium dark:bg-teal-800">
                          {grade.subject_code}
                        </div>
                        <div className="flex-grow bg-white p-1 text-center dark:bg-neutral-950">
                          {grade.subject_name ?? "-- Unknown --"}
                        </div>
                        <div className="w-[10%] bg-yellow-50 p-1 text-center dark:bg-yellow-950">
                          {grade.credits}
                        </div>
                        <div className="w-[12%] bg-emerald-300 p-1 px-2 text-left font-medium dark:bg-emerald-700">
                          {grade.grade}
                        </div>
                      </div>
                    ))}
                </div>
                <div className="font-geist flex w-full divide-x-2 divide-black bg-amber-100 dark:bg-amber-900">
