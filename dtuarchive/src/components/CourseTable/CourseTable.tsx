...
const sortingHeader = (
  columnName: string
): React.FC<{ column: Column<CourseTableData, unknown> }> => {
  const header = ({ column }: { column: Column<CourseTableData> }) => (
    <Button
      variant="ghost"
...
export default function CourseTable(props: CourseTableProps) {
  ...
  const columnHelper = createColumnHelper<CourseTableData>()

  const columns: ColumnDef<CourseTableData, string>[] = [
    {
      id: "index",
      header: "Index",
      size: 10
